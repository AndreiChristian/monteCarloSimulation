...
icecream_exprenses = np.array([300,200,150,400,200])
...
iceceam_mean = np.mean(icecream_exprenses)
iceceam_std_dev = np.std(icecream_exprenses)
...
for i in range(num_iterations):
    ...
    simulated_icecream_cost = np.random.normal(iceceam_mean, iceceam_std_dev)
    simulated_profit = simulated_revenue - simulated_rent - simulated_icecream_cost
...
naive = np.mean(competitors_revenue) - np.mean(icecream_exprenses) - np.mean(rent_expense)

print("naive version", naive)